...
def get_dashboard_summary(db: Session) -> DashboardSummaryResponse:
    try:
        ...
        success_runs_24h = (
            db.scalar(select(func.count(TaskRun.id)).where(and_(TaskRun.status == "success", TaskRun.created_at >= since_24h)))
            or 0
        )
        failed_runs_24h = (
            db.scalar(select(func.count(TaskRun.id)).where(and_(TaskRun.status == "failed", TaskRun.created_at >= since_24h)))
            or 0
        )
...
def get_dashboard_jobs() -> list[dict[str, str | int | bool | None]]:
    try:
        ...
        logger.exception("Dashboard jobs aggregation failed")
        raise
        ...
        for task in html_tasks:
            ...
            items.append(
                DashboardHtmlResultItem(
                    ...
                    preview_text=build_preview_text(preview_source),
